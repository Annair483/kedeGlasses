<?php
    ...
    if($fieldName){
        if($sort==='asc'){
            $gdsList = $conn -> query('select * from goodslist ORDER BY ('.$fieldName.'+0)');
        }else if($sort==='desc'){
            $gdsList = $conn -> query('select * from goodslist ORDER BY ('.$fieldName.'+0)'.$sort);
        }
    }else{
        $gdsList = $conn -> query('select * from goodslist');
    }
    //  列表页数据
